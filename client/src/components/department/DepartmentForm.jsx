...
const DepartmentForm = () => {
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((res) => {
        ...
        if (id) {
          axios
            .get(`http://localhost:3001/api/departments/${id}`, config)
            .then((res) => {
              setFormData({
                ...res.data,
                employees: res.data.employees.map((employee) => employee._id),
              });
              setSelectedEmployees(
                res.data.employees.map((employee) => employee._id)
              );
            })
            .catch((error) =>
              console.error(
  ...
  const handleChange = (e) => {
    ...
    if (checked) {
      ...
      setSelectedEmployees((prevSelected) =>
        prevSelected.filter((empId) => empId !== value)
      );
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (id) {
        await axios.put(
          `http://localhost:3001/api/departments/${id}`,
          updatedFormData,
          {
            headers: {
              Authorization: `Bearer ${authState.user.token}`,
          ...
          <div>
            ...
            <ul>
              {selectedEmployees.map((employeeId) => (
                <li>
                  <div key={employeeId}>
                    {users.find((user) => user._id === employeeId)?.username}
                  </div>
                </li>
              ))}
            </ul>
